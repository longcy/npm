...
var ManifestProcessor = /** @class */ (function (_super) {
    ...
    function ManifestProcessor(manifest) {
        ...
        _this.vsix = __assign({}, _this.vsix, { id: manifest.name, displayName: manifest.displayName || manifest.name, version: manifest.version, publisher: manifest.publisher, engine: manifest.engines['vscode'], description: manifest.description || '', categories: (manifest.categories || []).join(','), flags: flags.join(' '), links: {
            ...
            customerQnALink: customerQnALink, extensionDependencies: _(manifest.extensionDependencies || []).uniq().join(',') });
        if (isGitHub) {
    ...
    ManifestProcessor.prototype.onEnd = function () {
        ...
        if (!this.manifest.repository) {
            ...
            promise = util.read('Do you want to continue? [y/N] ')
                .then(function (answer) { return /^y$/i.test(answer) ? Promise.resolve() : Promise.reject('Aborted'); });
        }
        return promise;
    };
...
var TagsProcessor = /** @class */ (function (_super) {
    ...
    TagsProcessor.prototype.onEnd = function () {
        ...
        var trimmedKeywords = keywords.filter(function (keyword, index) { return index < 5 || keyword === 'multi-root ready'; });
        ...
        return promise.then(function (keywords) {
            ...
            keywords = keywords.concat(colorThemes, iconThemes, snippets, keybindings, debuggers, json, localizationContributions, languageContributions, languageActivations, grammars, descriptionKeywords);
...
function collectAllFiles(cwd, useYarn) {
    if (useYarn === void 0) { useYarn = false; }
    return npm_1.getDependencies(cwd, useYarn).then(function (deps) {
...
function collectFiles(cwd, useYarn) {
    if (useYarn === void 0) { useYarn = false; }
    return collectAllFiles(cwd, useYarn).then(function (files) {
...
function collect(manifest, options) {
    ...
    return collectFiles(cwd, useYarn).then(function (fileNames) {
 ...
 */
function listFiles(cwd, useYarn) {
    if (cwd === void 0) { cwd = process.cwd(); }
    if (useYarn === void 0) { useYarn = false; }
    return readManifest(cwd)
        .then(function (manifest) { return collectFiles(cwd, useYarn); });
}
exports.listFiles = listFiles;
/**
 * Lists the files included in the extension's package. Runs prepublish.
 */
function ls(cwd, useYarn) {
    if (cwd === void 0) { cwd = process.cwd(); }
    if (useYarn === void 0) { useYarn = false; }
    return readManifest(cwd)
        .then(function (manifest) { return prepublish(cwd, manifest); })
